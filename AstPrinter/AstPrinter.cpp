...
public:
    std::string parenthesize(std::string name, std::vector<std::unique_ptr<Expr<std::string>>>& exprs) const {
    ...
    std::string print(Expr<std::string>* expr) const {
        return expr->accept(*this);
    }

    std::string visitBinaryExpr(Binary<std::string>& expr) const override {
    ...
    std::string visitGroupingExpr(Grouping<std::string>& expr) const override {
        std::vector<std::unique_ptr<Expr<std::string>>> vect;
        vect.push_back(std::move(expr.expression));
        return parenthesize("group", vect);
    }

    std::string visitLiteralExpr(Literal<std::string>& expr) const override { 
    ...
    std::string visitUnaryExpr(Unary<std::string>& expr) const override {
        std::vector<std::unique_ptr<Expr<std::string>>> vect;
